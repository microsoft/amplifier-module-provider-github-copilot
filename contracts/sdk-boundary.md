...
## Version
- **Current:** 1.15 (SDK v1.0.14 — ToolForwarding hardened: `SDKToolWrapper.is_terminal` added to mirror the SDK's new `copilot.tools.Tool.is_terminal` field, read during tool-definition serialization on both the create and resume paths; MinimalMode extended to MUST:1-18)
- **Module Reference:** amplifier_module_provider_github_copilot/sdk_adapter/
- **Status:** Non-Negotiable Constraint
- **Update (v1.15):** 2026-09-24 — SDK bumped v1.0.7 → v1.0.14 (stable; bundled CLI 1.0.77). The 5 quarantined imports are unchanged in name, location and shape, and the `1.0.8 → 1.0.14` span removes/renames NOTHING: `CopilotClient.__init__`, `create_session` and `CopilotSession.send` are each purely additive (verified by signature set-difference — removed == none for all three). Two hardening changes. (1) ToolForwarding: the SDK's `copilot.tools.Tool` gained `is_terminal: bool = False` (installed v1.0.14 `tools.py:93`), read when building tool wire-definitions on BOTH paths — create (`client.py:2570`) and resume (`client.py:3355`): `if tool.is_terminal: definition["isTerminal"] = True`. The duck-typed `SDKToolWrapper` lacked it, so every tool-forwarding turn would raise `AttributeError` — the identical failure mode to `defer` (v1.12) and `metadata` (v1.14). Note the guard is a bare TRUTHINESS read, unlike the `is not None` tests used by its two predecessors; `False` default omits the `"isTerminal"` wire key (byte-identical pre-v1.0.14 payload). Amplifier owns turn control at the kernel layer and never delegates turn termination to the SDK. (2) MinimalMode extended from MUST:1-16 to MUST:1-18: v1.0.14 adds two mode-gated `create_session` capabilities — `custom_agents_local_only` (empty-mode default `True`) and `enable_experimental_mode` (empty-mode default `False`). Under `mode="copilot-cli"` both helpers return `None`, so unpinned they inherit bundled-CLI defaults; `enable_experimental_mode` in particular means a CLI upgrade could silently switch experimental behavior on underneath the provider. Both pinned to mirror the SDK's own empty-mode defaults. Companion changes: `ReasoningEffort` grows to 5 members (`"max"` becomes SDK-native — the provider's forwarded set is UNCHANGED, `"max"` merely moves from documented fallback-allowlist extra to SDK member; `"none"` remains the sole extra), and 27 new session-event types classified DROP (config-driven, event-vocabulary).
- **Update (v1.14):** 2026-07-22 — ToolForwarding hardened for v1.0.7. The SDK's `copilot.tools.Tool` gained a `metadata: dict[str, Any] | None` field (installed v1.0.7 `tools.py:78`, docstring `tools.py:186`: "Opaque, host-defined metadata associated with the tool definition"); the SDK reads it when building tool wire-definitions on BOTH paths — create (`client.py:2182-2183`) and resume (`client.py:2858-2859`): `if tool.metadata is not None: definition["metadata"] = tool.metadata`. The provider's duck-typed `SDKToolWrapper` lacked it, so every tool-forwarding turn would raise `AttributeError: 'SDKToolWrapper' object has no attribute 'metadata'` — the exact `defer` failure mode from v1.12, reachable only by an E2E that exercises the real tool-definition builder (not the mocked unit suite or tool-less live smoke). `SDKToolWrapper.metadata` added, defaulting to `None` (omits the wire key = byte-identical pre-v1.0.7 payload); Amplifier attaches no host-defined tool metadata. The existing Tier-6 structural guard (`test_sdk_assumptions.py`) already fails RED on any unmirrored SDK `Tool` field; a v1.0.7 wire-omission regression test (`tests/test_sdk_boundary_contract.py::test_wrapper_survives_sdk_v107_tool_definition_build`) additionally pins that `metadata is None` omits the key. Also: 6 new v1.0.7 SDK session-event types classified as DROP (config-driven), pinned to exact DROP in `tests/test_event_classifications.py`.
...
- **History:**
  - **1.15** — SDK v1.0.14: ToolForwarding:MUST:2 extended — `SDKToolWrapper` gains `is_terminal: bool = False`, mirroring the SDK's own `copilot.tools.Tool.is_terminal` (installed v1.0.14 `tools.py:93`). The SDK reads `tool.is_terminal` building tool wire-definitions on BOTH the create (`client.py:2570`) and resume (`client.py:3355`) paths; the duck-typed wrapper lacking it would raise `AttributeError` on every tool-forwarding turn — the identical failure mode to `defer` (v1.12) and `metadata` (v1.14), unreachable by the mocked unit suite or tool-less live smoke, only by an E2E exercising the real tool-definition builder. The read is a bare truthiness gate (NOT `is not None`, unlike its two predecessors), so the `False` default omits the `"isTerminal"` wire key (byte-identical pre-v1.0.14 payload); Amplifier owns turn control at the kernel layer and never delegates turn termination. MinimalMode:MUST:17-18 added — `custom_agents_local_only=True` and `enable_experimental_mode=False`, the 11th and 12th SDK mode-gated capability defaults, each mirroring the SDK empty-mode default; under `mode="copilot-cli"` both helpers return `None` so the bundled-CLI default would otherwise apply. Guards: the durable Tier-6 field-set introspection (`tests/test_sdk_assumptions.py`) fails RED on any future unmirrored SDK `Tool` field; `tests/test_sdk_boundary_contract.py::test_wrapper_survives_sdk_v1014_tool_definition_build` replays the v1.0.14 definition loop and pins `is_terminal False ⇒ key omitted`; `test_no_unpinned_sdk_mode_gated_capability` runtime-enumerates `_mode` so a future 13th default fails loudly. Companion changes: `ReasoningEffort` Literal grows to 5 members (`"max"` SDK-native; provider forwarded set unchanged) and 27 new v1.0.14 session-event types classified DROP (config-driven, event-vocabulary), pinned in `tests/test_event_classifications.py`.
  - **1.14** — SDK v1.0.7: ToolForwarding:MUST:2 extended — `SDKToolWrapper` gains `metadata: dict[str, Any] | None = None`, mirroring the SDK's own `copilot.tools.Tool.metadata` (installed v1.0.7 `tools.py:78`). The SDK reads `tool.metadata` building tool wire-definitions on BOTH the create (`client.py:2182-2183`) and resume (`client.py:2858-2859`) paths; the duck-typed wrapper lacking it would raise `AttributeError` on every tool-forwarding turn — the identical failure mode to `defer` (v1.12), unreachable by the mocked unit suite or tool-less live smoke, only by an E2E exercising the real tool-definition builder. `None` default omits the `"metadata"` wire key (byte-identical pre-v1.0.7 payload); Amplifier attaches no opaque host-defined tool metadata. Guards: the durable Tier-6 field-set introspection (`tests/test_sdk_assumptions.py`) fails RED on any future unmirrored SDK `Tool` field; `tests/test_sdk_boundary_contract.py::test_wrapper_survives_sdk_v107_tool_definition_build` replays the v1.0.7 definition loop and pins `metadata is None ⇒ key omitted`. Companion event change: 6 new v1.0.7 session-event types classified DROP (config-driven, event-vocabulary), pinned to exact DROP in `tests/test_event_classifications.py`.
...
| sdk-boundary:MinimalMode:MUST:17 | `custom_agents_local_only` MUST be set to `True` | Confines SDK custom-agent resolution to the local session — never resolves agents from shared/remote sources. Complements MUST:5 (`custom_agents=[]`), which only empties the list. Pinned because `mode="copilot-cli"` does not invoke the empty-mode default helper (`_custom_agents_local_only_default` returns `True` only when `mode == "empty"`); mirrors that empty-mode default exactly. |
| sdk-boundary:MinimalMode:MUST:18 | `enable_experimental_mode` MUST be set to `False` | Keeps SDK experimental features off — Amplifier owns the provider's behavioral surface. Pinned because `mode="copilot-cli"` does not invoke the empty-mode default helper (`_enable_experimental_mode_default` returns `False` only when `mode == "empty"`); unpinned, a bundled-CLI upgrade could silently switch experimental behavior on underneath the provider. Mirrors the SDK empty-mode default exactly. |

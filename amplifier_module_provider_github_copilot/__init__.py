...
def _check_sdk_version(version_str: str) -> None:
    ...
    The exact pyproject pin (``==1.0.14``) is enforced separately by
    ...
    if _parse_sdk_version(version_str) < _SDK_FLOOR:
        raise ImportError(
            ...
            "Pinned target: ==1.0.14 (pyproject.toml). "
            "Reinstall with: pip install 'github-copilot-sdk==1.0.14' "
...
if not _SKIP_SDK_CHECK:  # pragma: no cover
    try:
        ...
        raise ImportError(
            ...
            "Install with:  pip install 'github-copilot-sdk==1.0.14'"
        ) from _e
